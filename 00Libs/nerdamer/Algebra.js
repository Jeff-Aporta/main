...
(function () {
  ...
  Polynomial.prototype = {
    ...
    parse: function (symbol, c) {
      ...
      if (!symbol.isPoly())
        throw core.exceptions.NerdamerTypeError(
    ...
    equalsNumber: function (x) {
      this.trim();
      return (
        this.coeffs.length === 1 && this.coeffs[0].toDecimal() === String(x)
      );
    },
    toString: function () {
  ...
  Symbol.prototype.coeffs = function (c, with_order) {
    ...
    if (s.isComposite()) {
      ...
      if (with_order)
        ...
        if (s.group === ES_PRODUCTO && s.isImaginary()) {
  ...
  core.Utils.subFunctions = function (symbol, map) {
    ...
    if (symbol.group === ES_SUMA || symbol.group === PL)
      return symbol.altVar(core.Utils.inBrackets(subbed.join("+")));
    if (symbol.group === ES_PRODUCTO)
      return symbol.altVar(core.Utils.inBrackets(subbed.join("*")));
    return symbol.text();
  };
  ...
  var __ = (core.Algebra = {
    ...
    Factor: {
      ...
      zeroes: function (symbol, factors) {
        ...
        try {
          ...
          if (core.Utils.allSame(powers)) {
            ...
            if (n_zeroes === 3) {
              p = new Frac(
                powers[0] / Math.round((Math.sqrt(8 * n_terms - 1) - 3) / 2)
              );
            }
            /*
          ...
          if (sum.group !== ES_SUMA) return symbol; //nothing to do
          ...
          retval.each(function (x) {
            ...
            if (factored.group === ES_PRODUCTO) {
              ...
              factored.each(function (y) {
                ...
                if (_factored.group === ES_PRODUCTO) {
          ...
          if (cf.group === ES_PRODUCTO) {
            ...
            if (root.length === 1) {
              ...
              if (core.Utils.isInt(root2)) {
                //we found them both
                factors.add(
                  _.parse(format("({0})*({1})+({2})", symbols[1], v, root2))
                );
                factors.add(
                  _.parse(format("({0})*({1})+({2})", symbols[0], v, root1))
                );
                symbol = new Symbol(1);
              }
            }
          ...
          if (symbols.length === 2) {
            ...
            if (a.isCube() && b.isCube()) {
              ...
              if (sign_a === 1 && sign_b === -1) {
                ...
                factors.add(
                  _.parse(format("(({0})^2+({0})*({1})+({1})^2)", x, y))
                );
                symbol = Symbol(1);
      ...
      _factor: function (symbol, factors) {
        ...
        if (symbol.group === ES_SUMA) {
          ...
          symbol.each(function (x) {
            if (
              (x.group === ES_SUMA && x.power.greaterThan(1)) ||
              x.group === ES_PRODUCTO
            )
              x = _.expand(x);
            t = _.add(t, x);
        ...
        try {
          ...
          if (isInt(p) && !(p.lessThan(0) && symbol.group === FN)) {
            ...
            if (multiVar) {
              ...
              if (all_S && all_unit) {
                return _.pow(
                  _.parse(symbol, core.Utils.getFunctionsSubs(map)),
                  _.parse(p)
                );
              }
            }
      ...
      reduce: function (symbol, factors) {
        if (symbol.group === ES_SUMA && symbol.length === 2) {
      ...
      coeffFactor: function (symbol, factors) {
        if (symbol.isComposite()) {
          ...
          if (!gcd.equals(1)) {
            ...
            var terms = symbol
              .collectSymbols(null, null, null, true)
              .sort(function (a, b) {
          ...
          for (var i = 0; i < roots.length; i++) {
            ...
            if (!isNaN(r)) {
              ...
              for (var x in cfactors) {
                ...
                var n = core.Utils.round(
                  Math.log(x) / Math.log(Math.abs(r)),
                  8
                );
                if (isInt(n)) {
      ...
      mfactor: function (symbol, factors) {
        if (symbol.group === FN) {
          ...
          for (var x in sorted) {
            ...
            var neg_numeric_factor =
              isInt(new_factor) && new_factor.lessThan(0);
      ...
      if (g === PL || g === ES_SUMA) {
        ...
        for (var s in e.symbols) {
          ...
          if (sg === ES_PRODUCTO) {
            ...
            if (sg === PL || sg === ES_SUMA) status = __.isLinear(symbol);
            else {
    ...
    gcd_: function (a, b) {
      ...
      var den = _.multiply(
        a.getDenom() || new Symbol(1),
        b.getDenom() || new Symbol(1)
      ).invert();
      ...
      if (a.group === ES_PRODUCTO || b.group === ES_PRODUCTO) {
    ...
    divide: function (symbol1, symbol2) {
      ...
      if (!den.isConstant("all")) {
        symbol1 = _.expand(
          Symbol.unwrapPARENS(_.multiply(factored, den.clone()))
        );
      }
    ...
    div: function (symbol1, symbol2) {
      ...
      try {
        ...
        if (symbol1.group === S && symbol2.group === ES_SUMA) {
          var x = symbol1.value;
          ...
          if (
            ...
            return [
              _.divide(k.clone(), f.a.clone()),
              _.divide(_.multiply(k, f.b), f.a).negate(),
            ];
          }
        }
        ...
        if (vars.length === 1) {
          ...
          if (det) {
            ...
            while (dividend_larger && can_divide(s1, s2)) {
              if (safety++ > max) {
                throw new core.exceptions.InfiniteLoopError(
                  "Unable to compute!"
                );
              }
    ...
    degree: function (symbol, v, o) {
      ...
      } else if (symbol.group === ES_PRODUCTO) {
        symbol.each(function (x) {
          o.depth++;
          __.degree(x, v, o);
          o.depth++;
    ...
    Simplify: {
      ...
      complexSimp: function (num, den) {
        ...
        return _.divide(
          _.add(
            _.add(ac, bd),
            _.multiply(_.subtract(bc, ad), Symbol.imaginary())
          ),
          cd
        );
      },
      simplificacion_trigonometrica: function (symbol) {
        console.log(nerdamer(symbol))
        if (symbol.containsFunction(["cos", "sin", "tan","sec","csc","cot"])) {
          symbol = symbol.clone();
          ...
          if (symbol.group === ES_SUMA) {
            ...
            symbol.each(function (x) {
              ...
              var tr = __.Simplify.simplificacion_trigonometrica(
                x.fnTransform()
              );
              sym = _.add(sym, tr);
            }, true);
            ...
            retval = _.pow(
              _.multiply(new Symbol(symbol.multiplier), sym),
              new Symbol(symbol.power)
            );
          } else if (symbol.group === ES_PRODUCTO) {
            if (retval.group === ES_PRODUCTO) {
              var t = new Symbol(1);
          ...
          if (n.fname === "sin" && d.group === N && n.power.lessThan(0)) {
            retval = _.parse(
              core.Utils.format(
                "({0})*({1})*sec({2})^({3})",
                d.multiplier,
      ...
      simplify: function (symbol) {
        ...
        simplified = __.Simplify.simplificacion_trigonometrica(simplified);
