...
(function () {
  var core = nerdamer.getCore(),
    ...
    ES_NUMERO = core.groups.N,
    ES_SIMPLE_VARIABLE = core.groups.S,
    ES_FUNCIÓN = core.groups.FN,
    ES_POTENCIAS_DIFERENTES_EN_TERMINOS = core.groups.PL,
    ES_SUMA = core.groups.CP,
    ES_MULTIPLICACIÓN = core.groups.CB,
    ES_EXPONENCIAL = core.groups.EX,
    ES_POTENCIA_FRACCIONARIA = core.groups.P,
    LOG = Settings.LOG,
    EXP = "exp",
    ABS = "abs",
    SQRT = "sqrt",
  ...
  Symbol.prototype.fnTransform = function () {
    if (this.group !== ES_FUNCIÓN) return this;
    var retval,
      a = this.args[0];
    ...
    if (this.isLinear()) {
      ...
      retval = _.parse(
        format("csc({0})^(-({1}))", this.args[0], this.power.negate())
      );
    } else if (this.fname === COS && this.power.lessThan(0)) {
      retval = _.parse(
        format("sec({0})^(-({1}))", this.args[0], this.power.negate())
      );
  ...
  Symbol.prototype.hasTrig = function () {
    if (this.isConstant(true) || this.group === ES_SIMPLE_VARIABLE)
      return false;
    if (
      this.fname &&
      (core.Utils.in_trig(this.fname) || core.Utils.in_inverse_trig(this.fname))
    )
      return true;
    if (this.symbols) {
  ...
  core.Utils.toCommonDenominator = function (symbol) {
    ...
    if (symbol.isComposite() && symbol.isLinear()) {
      ...
      var retval = _.multiply(
        m,
        core.Algebra.divide(_.expand(numerator), _.expand(denominator))
      );
      return retval;
    }
    return symbol;
  ...
  var all_functions = (core.Utils.all_functions = function (arr) {
      for (var i = 0, l = arr.length; i < l; i++)
        if (arr[i].group !== ES_FUNCIÓN) return false;
      return true;
    }),
  ...
  var __ = (core.Calculus = {
    sum: function (fn, index, start, end) {
      if (!(index.group === core.groups.S))
        throw new core.exceptions.NerdamerTypeError(
          "Index must be symbol. " + text(index) + " provided"
        );
      index = index.value;
      var retval;
      if (
    ...
    diff: function (entrada, var_derivación, orden_derivada, subderivada = 0) {
      ...
      var VARIABLE_DERIVACIÓN = isSymbol(var_derivación)
        ? var_derivación.text()
        : var_derivación;

      //the nth derivative
      orden_derivada = isSymbol(orden_derivada)
        ? orden_derivada.multiplier
        : orden_derivada || 1;

      if (!VARIABLE_DERIVACIÓN) {
        let variables = core.Utils.variables(entrada);
        variables.sort();
        for (const v of variables) {
          if (v == "x" || v == "y" || v == "z") {
            VARIABLE_DERIVACIÓN = v;
          }
        }
        if (!VARIABLE_DERIVACIÓN) {
      ...
      if (entrada.group === ES_FUNCIÓN && entrada.fname === SQRT) {
        ...
        if (
          argumento.group !== ES_NUMERO ||
          argumento.group !== ES_POTENCIA_FRACCIONARIA
        ) {
          argumento.power = isSymbol(argumento.power)
            ? _.multiply(
                argumento.power,
                _.multiply(new Symbol(1 / 2)),
                potencia
              )
            : argumento.power.multiply(new Frac(0.5)).multiply(potencia);
      ...
      function DERIVADA_POTENCIA(symbol) {
        if (
          symbol.value === VARIABLE_DERIVACIÓN ||
          symbol.contains(VARIABLE_DERIVACIÓN, true)
        ) {
      ...
      function derive(symbol, subderivada = 0) {
        var g = symbol.group,
          a,
          b,
          cp;

        let origin = symbol.text();

        if (
          g === ES_NUMERO ||
          (g === ES_SIMPLE_VARIABLE && symbol.value !== VARIABLE_DERIVACIÓN) ||
          g === ES_POTENCIA_FRACCIONARIA
        ) {
          symbol = Symbol(0);
          ...
          switch (symbol.fname) {
            ...
            case ATAN:
              ...
              break;
            case ABS:
              COHEFICIENTE = symbol.multiplier.clone();
              symbol.toUnitMultiplier();
              ...
              symbol.multiplier = COHEFICIENTE;
              break;
            case "parens":
            ...
            case "sinc":
              symbol = _.parse(
                format("(({0})*cos({0})-sin({0}))*({0})^(-2)", symbol.args[0])
              );
              break;
            case Settings.LOG10:
              symbol = _.parse(
                "1/((" + symbol.args[0] + ")*" + Settings.LOG + "(10))"
              );
              break;
            default:
              symbol = _.symfunction("diff", [symbol, var_derivación]);
          }
        } else if (
          g === ES_EXPONENCIAL ||
          (g === ES_FUNCIÓN && isSymbol(symbol.power))
        ) {
          var value;
          if (g === ES_EXPONENCIAL) {
            value = symbol.value;
          } else if (g === ES_FUNCIÓN && symbol.contains(VARIABLE_DERIVACIÓN)) {
          ...
          b.toLinear();
          b.toUnitMultiplier();
          symbol = _.multiply(DERIVADA_POTENCIA(symbol.clone()), derive(b));
        } else if (g === ES_SUMA || g === ES_POTENCIAS_DIFERENTES_EN_TERMINOS) {
          ...
          for (var x in symbol.symbols) {
            result = _.add(
              result,
              __.diff(symbol.symbols[x].clone(), VARIABLE_DERIVACIÓN)
            );
          }
          symbol = _.multiply(DERIVADA_POTENCIA(c), result);
        }
      ...
      function DERIVADA_DE_PRODUCTO(symbol) {
        ...
        for (var i = 0; i < l; i++) {
          var df = __.diff(symbols[i].clone(), VARIABLE_DERIVACIÓN);
          for (var j = 0; j < l; j++) {
    ...
    integration: {
      u_substitution: function (symbols, dx) {
        ...
        var a = symbols[0].clone(),
          ...
          u = "__u__",
          Q;
        if (g1 === ES_FUNCIÓN && g2 !== ES_FUNCIÓN) {
          //e.g. 2*x*cos(x^2)
          var arg = a.args[0];
          ...
          return retval;
        } else if (g2 === ES_EXPONENCIAL && g1 !== ES_EXPONENCIAL) {
          var p = b.power;
      ...
      partial_fraction: function (input, dx, depth, opt) {
        ...
        if (
          partial_fractions.group === ES_MULTIPLICACIÓN &&
          partial_fractions.isLinear()
        ) {
          ...
          result = _.add(
            result,
            __.integrate(partial_fractions, dx, depth, opt)
          );
        }
        return result;
      },
      get_udv: function (symbol) {
        ...
        var setSymbol = function (x) {
          ...
          if (g === ES_FUNCIÓN) {
            ...
            g === ES_SIMPLE_VARIABLE ||
            (x.isComposite() && x.isLinear()) ||
            (g === ES_MULTIPLICACIÓN && x.isLinear())
          ) {
            parts[2].push(x);
          } else if (g === ES_EXPONENCIAL || (x.isComposite() && !x.isLinear()))
            parts[4].push(x);
          else __.integration.stop();
        };

        if (symbol.group === ES_MULTIPLICACIÓN)
          symbol.each(function (x) {
      ...
      return core.Utils.block(
        ...
        function () {
          ...
          try {
            ...
            //e.g. 2*x
            ...
            } else if (g === ES_EXPONENCIAL) {
              if (
                symbol.previousGroup === ES_FUNCIÓN &&
                !(
                  symbol.fname === "sqrt" ||
                  symbol.fname === Settings.PARENTHESIS
                )
              )
                __.integration.stop();
              //check the base
              if (symbol.contains(dx) && symbol.previousGroup !== ES_FUNCIÓN) {
              ...
              //e.g. a^x or 9^x
                ...
                if (a.contains(dx)) {
                  ...
                  if (x.group === ES_SIMPLE_VARIABLE && x.isLinear()) {
                ...
                if (symbol.isE()) {
                  if (a.isLinear()) retval = symbol;
                    ...
                    if (
                      a.isE() &&
                      a.power.group === ES_SIMPLE_VARIABLE &&
              ...
              if (symbol.power.equals(1)) {
                ...
                if (
                  p === -1 &&
                  x.group !== ES_POTENCIAS_DIFERENTES_EN_TERMINOS &&
                  x.power.equals(2)
                ) {
                  var b_is_positive = isInt(b) ? b > 0 : true;
                  //we can now check for atan
                  if (
                    x.group === ES_SIMPLE_VARIABLE &&
                    x.power.equals(2) &&
                    b_is_positive
                  ) {
                    ////then we have atan
                    ...
                    retval = _.divide(f, d);
                  } else if (x.group === ES_SIMPLE_VARIABLE && x.isLinear()) {
                  ...
                  if (x.group === ES_SIMPLE_VARIABLE && x.power.equals(2)) {
                    if (
                  ...
                  if (
                    ...
                    if (
                      ...
                      if (
                        core.Algebra.degree(f, _.parse(dx)).equals(2) &&
                        !was_factored
                      ) {
                        try {
                          var f1, fx, u, sq;
              ...
              if (
                ...
                if (!arg.isComposite())
                  retval = _.multiply(
                    _.parse(m),
                    __.integration.by_parts(symbol, dx, depth, opt)
                  );
                else {
                  //integral u du
                ...
                if (!a.contains(dx, true) && symbol.isLinear()) {
                  ...
                  if (fname === ABS) {
                    ...
                    if (
                      !(
                        ag === ES_SUMA ||
                        ag === ES_SIMPLE_VARIABLE ||
                        ag === ES_MULTIPLICACIÓN
                      ) ||
                    ...
                    /**TODO**/ switch (fname) {
                      ...
                      case TAN:
                        retval = _.parse(
                          format(Settings.LOG + "(sec({0}))", arg)
                        );
                        break;
                      case SEC:
                        retval = _.parse(
                          format(Settings.LOG + "(tan({0})+sec({0}))", arg)
                        );
                        break;
                      case CSC:
                        retval = _.parse(
                          format(
                            "-" + Settings.LOG + "(csc({0})+cot({0}))",
                            arg
                          )
                        );
                        break;
                      ...
                      case TANH:
                        retval = _.parse(
                          format(Settings.LOG + "(cosh({0}))", arg)
                        );
                        break;
                      case ASEC:
                      ...
                      case CSCH:
                        retval = _.parse(
                          format(Settings.LOG + "(tanh(({0})/2))", arg)
                        );
                        break;
                      case COTH:
                        retval = _.parse(
                          format(Settings.LOG + "(sinh({0}))", arg)
                        );
                        break;
                      //end htrig
                      case EXP:
            ...
            } else if (g === ES_POTENCIAS_DIFERENTES_EN_TERMINOS) {
              retval = __.integration.partial_fraction(symbol, dx, depth);
            } else if (g === ES_MULTIPLICACIÓN) {
              var den = symbol.getDenom();
              if (den.group === ES_SIMPLE_VARIABLE) symbol = _.expand(symbol);
              ...
              if (cfsymbol.group !== ES_MULTIPLICACIÓN) {
                ...
                //otherwise the denominator is one lumped together symbol
                  ...
                  if (l === 2) {
                    ...
                    if (!retval) {
                      ...
                      if (g1 === ES_FUNCIÓN && g2 === ES_FUNCIÓN) {
                        if (fn1 === LOG || fn2 === LOG) {
                          ...
                          if (
                            ...
                            !(
                              arg1.group === ES_SUMA ||
                              arg1.group === ES_MULTIPLICACIÓN ||
                              arg1.group === ES_SIMPLE_VARIABLE
                            )
                          )
                            __.integration.stop();
                        ...
                        if (
                          ...
                          retval = __.integration.poly_integrate(
                            sym1,
                            dx,
                            depth
                          );
                        ...
                        if (
                          sym1.isE() &&
                          (sym1.power.group === ES_SIMPLE_VARIABLE ||
                            sym1.power.group === ES_MULTIPLICACIÓN) &&
                          sym2.power.equals(-1)
                        ) {
                        ...
                        g1 === ES_POTENCIAS_DIFERENTES_EN_TERMINOS &&
                        g2 === ES_SIMPLE_VARIABLE
                      ) {
                      ...
                      } else if (g1 === ES_SUMA && g2 === ES_SIMPLE_VARIABLE) {
                        var f = sym1.clone().toLinear(),
                          f_is_linear = core.Algebra.degree(
                            f,
                            _.parse(dx)
                          ).equals(1);
                        ...
                        if (sym1.power.equals(-1 / 2)) {
                          ...
                          if (
                            ...
                            var u_transform = function (f, u) {
                              var integral = _.parse(
                                __.integrate(f, dx, depth, opt).sub(
                                  dx,
                                  format(u, dx)
                                )
                              );
                          ...
                          if (
                            ...
                            if (div.group !== ES_MULTIPLICACIÓN) {
                              retval = new Symbol(0);
                              div.each(function (t) {
                                retval = _.add(
                                  retval,
                        ...
                        (g1 === ES_FUNCIÓN && g2 === ES_SUMA) ||
                        (g2 === ES_FUNCIÓN && g1 === ES_SUMA)
                      ) {
                        if (g2 === ES_FUNCIÓN && g1 === ES_SUMA) {
                          var t = sym1;
                          sym1 = sym2;
                          sym2 = t; //swap
                        }
                    ...
                    l === 3 &&
                    ((symbols[2].group === ES_SIMPLE_VARIABLE &&
                      symbols[2].power.lessThan(2)) ||
                      symbols[0].group === ES_SUMA)
                  ) {
                    var first = symbols[0];
                    if (first.group === ES_SUMA) {
    ...
    defint: function (symbol, from, to, dx) {
      ...
      if (!hasTrig && !integral.hasIntegral()) {
        ...
        retval = new Symbol(
          core.Math2.num_integrate(f, Number(from), Number(to))
        );
    ...
    Limit: {
      ...
      divide: function (f, g, x, lim, depth) {
        ...
        do {
          ...
          var indeterminate =
            (isInfinity(lim1) && isInfinity(lim2)) ||
            (equals(lim1, 0) && equals(lim2, 0));
        ...
        if (lim.isConstant(true) && den_is_zero) {
          retval = Symbol.infinity(
            core.Utils.even(p) && lim1.lessThan(0) ? -1 : undefined
          );
        } else if (den_is_zero) {
      ...
      rewriteToLog: function (symbol) {
        ...
        return _.pow(
          new Symbol("e"),
          _.multiply(p, _.symfunction(Settings.LOG + "", [symbol]))
        );
      },
      getSubbed: function (f, x, lim) {
        var retval;
        //1. rewrite EX with base e
        if (f.group === ES_EXPONENCIAL) {
          f = __.rewriteToLog(f);
        }
      ...
      isConvergent: function (limit) {
        ...
        if (
          ...
          (__.Limit.isInterval(limit) &&
            limit.elements[0].isInfinity &&
            limit.elements[1].isInfinity) ||
          ...
          if (symbol.isConstant(true)) {
            ...
            if (!retval) {
              ...
              if (den.isConstant(true)) {
                ...
                if (symbol.group === ES_EXPONENCIAL) {
                  //https://en.wikipedia.org/wiki/List_of_limits
                  ...
                  if (
                    _num.group === ES_SIMPLE_VARIABLE &&
                    _num.multiplier.isOne() &&
                    fn.ax.group === ES_SIMPLE_VARIABLE &&
                  ...
                  symbol.group === ES_FUNCIÓN &&
                  symbol.args.length === 1
                ) {
                  var evaluates;
                ...
                } else if (symbol.group === ES_SIMPLE_VARIABLE) {
                  if (symbol.power > 0)
                ...
                } else if (symbol.group === ES_MULTIPLICACIÓN) {
                  var lim1, lim2;
                  ...
                  for (var i = 0, l = symbols.length; i < l; i++) {
                    ...
                    if (
                      sym.group === ES_FUNCIÓN ||
                      (sym.group === ES_MULTIPLICACIÓN && sym.hasFunc())
                    ) {
                      fns = _.add(fns, sym);
